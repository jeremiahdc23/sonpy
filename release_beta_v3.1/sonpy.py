...
# New function: setOutput
# Deleted function: getOutput (depreciated, to avoid confusion)
...
class sonnet(object):
   ...
   # Tested on Windows, can be extended to Linux (but not Mac)
    ...
    ########################################################################
    # Set the output file format in the FILEOUT block.
    ...
    def setOutput(self, **kwargs):
        ...
        if dictParams["folder"] == "":
            self.setDataFilePath(self._sonnet_file_path + filename)
        else:
            self.setDataFilePath(self._sonnet_file_path + dictParams["folder"] + "\\" + filename)
    ...
    def plotData(self, ydata, run=1):
        ...
        # ydata is on form "MAG[S12]", "ANG[S12]" or an other valid string.
        ...
        if (file_found == 0):
            print("Project file %s can not be located in path %s"%(self._data_file,self._sonnet_data_file_path))
            raise self._exception("Sonnet project file not found! Check that directory and filename are correct!")
        ...
        with open(self._data_file_path + self._data_file, 'r') as fd:
            ...
            for line in fd:
                if line[0:9] == "Frequency" and currentRun < run:
                    ...
                    # Let's find the correct ydata row
                    params = line.split(",")
                    xindex = 0
                    yindex = params.index(ydata)
                    break

            # Jump to the first data line
            line = fd.readline()
            while line !=
